...
function tambahProduk($data)
{
    ...
    $query = "INSERT INTO products
                VALUES
                ('', '$name', '$unit', '$price', '$descriptions', '$category', '$stock','$id')
            ";
...
function checkout($data)
{
    ...
    $queryTransaction = "INSERT INTO transactions
                            VALUES
                            ('', '$user_id', '$tani', '$total_price', '$city', '$rekening', 
                                '$status', '$weight', '$delivered', '$photo', '$code', '', NULL, NOW())
                        ";
    ...
    $id_product = $cart["product_id"];
    $queryTransactionulasan = "INSERT INTO ulasan VALUES('', '$id_product', '$tani', '$rate', '$ulasan', '$transaction_id','$user_id','')";
    mysqli_query($conn, $queryTransactionulasan);
}
...
function ulasan($data)
{
    ...
    $query = "UPDATE ulasan SET rate ='$ratte',ulasan = '$msg', created = NOW() WHERE id_ulasan = $idulasan";
...
function tambahPetani($data)
{
    ...
    $phone_number = $data["phone_number"];
    $status = $data["status"];
    $alamat = $data["alamatdetail"];
    $kecamatan = $data["kecamatan"];

    $query = "INSERT INTO tani
                VALUES
                ('', '$name', '$email', '$password', '$phone_number', '$kecamatan', '$alamat', '$status',NOW())
            ";

    mysqli_query($conn, $query);
    return mysqli_affected_rows($conn);
}
function updatePetani($data)
{
    global $conn;
    $id = $data["id"];
    $result = query("SELECT password FROM tani WHERE id_tani = $id")[0];
    $name = $data["name"];
    $phone_number = $data["phone_number"];
    if (empty($data["password"])) {
        $password = $result["password"];
...
function tambahGaleriFarm($data)
{
    global $conn;
    $name = $data["name"];
    $photo = $data["photo"];
